...
import 'package:awj/src/features/suggest.dart';
...
class _ExploreTrailsScreenState extends State<ExploreTrailsScreen> {
  ...
  Future<void> fetchCities() async {
    try {
      ...
      for (var doc in snapshot.docs) {
        ...
        if (trailData.containsKey('City') && trailData['City'] is String) {
          uniqueCities.add(trailData['City']);
        }
      }
  ...
  backgroundColor: Colors.transparent, // Ensures custom Container background
  builder: (context) {
    return Padding(
      padding: MediaQuery.of(context).viewInsets,
      child: Container(
        decoration: BoxDecoration(
          color: const Color(0xFFEAE7D8), // Cream background color
          ...
          boxShadow: [
            BoxShadow(
              ...
              blurRadius: 8,
              offset: Offset(0, -4), // Shadow above the box
            ),
          ],
        ),
              ...
              child: Column(
                ...
                children: [
                  ...
                  buildFilterOption(
                    'Difficulty',
                    selectedDifficulty,
                    ['All', 'Easy', 'Medium', 'Difficult','Very Difficult'],
                    (value) {
                  ...
                  ElevatedButton(
                    ...
                    child: const Text(
                      'Apply Filters',
                      style: TextStyle(color: Color(0xFFEAE7D8)), // Cream text
                    ),
                  ),
                ],
              ),
    ...
    children: [
      ...
      Row(
        children: [
          ...
          SizedBox(width: 8), // Add space between search bar and filter icon
          // Filter Icon
          IconButton(
      ...
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          List<DocumentSnapshot> filteredDocuments = snapshot.data!.docs.where((doc) {
            ...
            double? distance;
            if (trailData['Distance'] is int || trailData['Distance'] is double) {
              distance = (trailData['Distance'] as num).toDouble();
            } else if (trailData['Distance'] is String) {
              distance = double.tryParse(trailData['Distance'].replaceAll(' km', ''));
            } else {
              distance = 0.0;
            }
            ...
            switch (selectedDistanceRange) {
              case '0-5 km':
                matchesDistanceRange = (distance != null && distance >= 0 && distance <= 5);
                break;
              case '5-10 km':
                matchesDistanceRange = (distance != null && distance > 5 && distance <= 10);
                break;
              case '10-15 km':
                matchesDistanceRange = (distance != null && distance > 10 && distance <= 15);
                break;
              case '15-20 km':
                matchesDistanceRange = (distance != null && distance > 15 && distance <= 20);
                break;
              default:
            ...
            // Filter conditions
            return (trailName.contains(searchQuery) || trailCity.contains(searchQuery)) &&
                (selectedDifficulty == 'All' || selectedDifficulty == difficulty) &&
                (selectedCity == 'All' || selectedCity.toLowerCase() == trailCity) &&
        ...
        child: FloatingActionButton(
          ...
          onPressed: () {
            if (widget.isGuest) {
              ...
              Navigator.push(
                context,
                MaterialPageRoute(builder: (context) => SuggestTrailPage()),
...
void toggleFavorite() async {
  ...
  try {
    ...
    final favoriteDoc = await favoritesCollection.doc(widget.documentId).get();
  ...
  final result = await Navigator.push(
    ...
    MaterialPageRoute(
      builder: (context) => ReviewDetailsPage(
        trailId: widget.documentId, // Pass required parameters
      ),
    ),
  );
              ...
              Positioned(
                ...
                right: 10,
                  child: IconButton(
                    ...
                    onPressed: () {
                      if (widget.isGuest) {
                        _showGuestMessageFavorite(); // Show a message if the user is a guest
                      } else {
                        toggleFavorite(); // Call the function to handle the favorite action
                      }
                    },
                  ),
              ),
